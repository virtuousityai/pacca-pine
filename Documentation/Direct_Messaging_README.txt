Direct Messaging with Pacca PINE and EMR Direct phiMail(R)
Version 1.3, 19 Jul 2014

A. Purpose: To provide a secure method from within Pacca PINE for sending/receiving
...
B. IMPORTANT:  Please be aware of the following limitations when using the Pacca PINE
Direct Messaging features with PHI in a production environment:

1. the current code only supports a single shared "group" Direct Address for each Pacca PINE
...
2. the current code only sends the CCR or CCD XML data that is already available in Pacca PINE;
these files as currently generated by existing Pacca PINE code do not meet the requirements
...
pane in the main Pacca PINE interface.

3. Log all data transmissions including date/time, patient, and whether transmission
was initiated by the patient through the Patient Portal or by an Pacca PINE user through the
main interface.

4. Receive Direct messages from other sources.

D. How it Works:
Once configured, Pacca PINE will interface with a phiMail Direct messaging server to complete the
...
E. What you need before enabling Direct Messaging in Pacca PINE:
...
configure Pacca PINE.

F. How to enable the Direct Messaging Features in Pacca PINE:
...
3. Specify the Pacca PINE user who will receive notification of new incoming Direct messages.
Enter their Pacca PINE username in the notification user field.
...
shorter interval. To disable automatic message checking through Pacca PINE's background service
...
Pacca PINE will operate in a receive-only mode.
...
presented by the phiMail server on the other side of Pacca PINE's connection.  Your Direct private
key and certificate are managed by the phiMail Server and are not installed in Pacca PINE.
Your Direct certificate is made available for your review by EMR Direct, but you will not
need to install it anywhere.

For added security, the trust anchor for the phiMail Server should be installed in the Pacca PINE
...
it begins with the text "-----BEGIN CERTIFICATE-----". Although Pacca PINE will connect
...
H. Checking the status and history of the Direct Messaging Service in Pacca PINE:
Administrators may view the status of the service by Selecting Reports::Services::Background
Services from the main Pacca PINE left navigation bar. The "View Log" link on this page or
...
is sent to a recipient using one of these other systems, Pacca PINE probably won't ever receive a
final delivery confirmation for that message.

J. How to use the Direct Messaging Features in Pacca PINE:
...
The Send button will be temporarily disabled while Pacca PINE is communicating with the phiMail
...
providers. Should you encounter a trust community with which Pacca PINE does not interoperate,
please let us know at support@emrdirect.com.

Pacca PINE will then display a status message immediately below the Address field, the
...
through the background services manager of another mechanism, Pacca PINE will automatically process
...
converted to separate Pacca PINE Documents, which will be referenced in the new Patient Note.
